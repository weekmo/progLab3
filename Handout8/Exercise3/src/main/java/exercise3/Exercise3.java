...
public class Exercise3 {
	public static void main(String[] args) {
		...
		Option method = new Option("m", "method", true, "Statistical format like: full, count, max or min!");
		...
		String methodValue = cmd.getOptionValue("method", "Watershed");
		
		//Select stats-format
		DescriptiveStatistics statistic=null;
		if (!cmd.hasOption("actin") && !cmd.hasOption("dna")) {
			double[] values = analyzImages("d", inFolder, outFolder,methodValue).getValues();
			statistic = analyzImages("f", inFolder, outFolder,methodValue);
			...
			if (cmd.hasOption("dna")) {
				statistic = analyzImages("d", inFolder, outFolder,methodValue);
			}
	 ...
	 */
	static DescriptiveStatistics analyzImages(String dna_actin,String inFolder,String outFolder,String method) {
		...
		for (int i = 0; i < files.length; i++) {
			...
			if (imageFile.isFile() && 
				...
				if(method.equalsIgnoreCase("Watershed")) {
					IJ.run(img, "Convert to Mask", "Black Background");
					Prefs.blackBackground = true;
					IJ.run(img, "Watershed", "only");
				}
				else if(method.equalsIgnoreCase("WekaSegmentation")) {
					WekaSegmentation ws = new WekaSegmentation(img);
					ws.loadClassifier("saved-model-file.model");
					img = ws.applyClassifier(img,0,false);
				}
				else {
					IJ.run(method , "number_of_classes=2" ) ;
				}
